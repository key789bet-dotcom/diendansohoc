...
router.post('/register', async (req,res)=>{
  ...
  try {
    ...
    const [insertRes] = await db.query('INSERT INTO users(username,email,password,ho_ten,phone,region) VALUES(?,?,?,?,?,?)',[username,email,hash,((ho||'')+' '+(ten||'')).trim(),phone||null,region||'mb']);
    const [newUser2] = await db.query('SELECT * FROM users WHERE id=?', [insertRes.insertId]);
